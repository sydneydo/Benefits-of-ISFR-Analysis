classdef PressureBalancer < handle
    ...
    methods
        %% Constructor
        function obj = PressureBalancer(modules)
        ...
        function obj = tick(obj)
            ...
            %             for j = 1:length(adj)
                ...
                for k = 1:length(sourceIndex)                   
                    ...
                    move(obj.Modules(sourceIndex(k)),obj.Modules(toNode(fromNode==sourceIndex(k))),pressureDiff(sourceIndex(k)))
                ...
                for m = 1:length(sinkIndex)
                    ...
                    if pressureDiff(toNode(fromNode==sinkIndex(m))) > 0
                        ...
                        if pressureDiff(toNode(fromNode==sinkIndex(m))) < abs(pressureDiff(sinkIndex(m)))
                            ...
                            move(obj.Modules(toNode(fromNode==sinkIndex(m))),obj.Modules(sinkIndex(m)),pressureDiff(toNode(fromNode==sinkIndex(m))));
                            ...
                            move(obj.Modules(toNode(fromNode==sinkIndex(m))),obj.Modules(sinkIndex(m)),abs(pressureDiff(sinkIndex(m))));
